...
,
  {
    ...
    "solveSteps": [
      {"step": "第1步·辨认定律", "detail": "看算式结构判断用哪个定律：只有加减→加法交换律/结合律；只有乘除→乘法交换律/结合律；有乘有加→乘法分配律。例如：25×4+25×6→分配律。"},
      {"step": "第2步·重组算式", "detail": "按定律规则重组：加法交换律 36+48=48+36；加法结合律 (23+45)+55=23+(45+55)；乘法分配律 (a+b)×c=a×c+b×c。例如：25×(4+6)=25×10=250。"},
      {"step": "第3步·巧算化简", "detail": "利用定律凑整：25×4=100、125×8=1000、99=100-1、101=100+1。例如：78×99=78×(100-1)=7800-78=7722。"},
      {"step": "第4步·反向验证", "detail": "用常规算法重算一遍验证结果。特别注意分配律的'括号外×括号内每一项'，容易漏乘最后一项。"}
    ],
    "commonMistakes": [
    ...
    "aiDiagnosis": {
      "trigger": "做错时弹出",
      "action": "运算定律卡住，通常不是算错而是用错定律。请先判断：①这个算式里只有加/减，还是只有乘/除，还是有乘+加？②如果是乘+加，必须用分配律，括号外乘括号里每一项。",
      "interactionType": "choice-question"
    }
  }
,
...
,
  {
    ...
    "solveSteps": [
      {"step": "第1步·整数化", "detail": "把两个小数当成整数列竖式，小数点先不管。例如：3.14×2.5 → 314×25=7850。"},
      {"step": "第2步·数小数位", "detail": "数两个因数各有几位小数：3.14有2位，2.5有1位，共3位小数。"},
      {"step": "第3步·定小数点", "detail": "从积的右边向左数出对应位数点小数点：7850 → 从右数3位 → 7.850 → 化简为7.85。"},
      {"step": "第4步·估算验证", "detail": "用近似值估算：3.14≈3，2.5≈2.5，3×2.5=7.5，答案7.85≈7.5，合理。"}
    ],
    "commonMistakes": [
    ...
    "aiDiagnosis": {
      "trigger": "做错时弹出",
      "action": "小数乘法出错，90%是因为小数位数数错了。请检查：①你把两个因数的小数位数加起来了吗？②积的小数点是从右边向左数的吗？③点完小数点后化简了末尾无意义的0吗？",
      "interactionType": "choice-question"
    }
  }
,
...
,
  {
    ...
    "solveSteps": [
      {"step": "第1步·分子乘分子", "detail": "两个分数的分子相乘作新分子。例如：3/4 × 2/5 → 新分子 = 3×2 = 6。"},
      {"step": "第2步·分母乘分母", "detail": "两个分数的分母相乘作新分母。例如：3/4 × 2/5 → 新分母 = 4×5 = 20，结果 = 6/20。"},
      {"step": "第3步·能约就约", "detail": "约分化简：分子分母同时除以最大公因数。6/20 → 分子分母同除以2 → 3/10。也可先约后乘更简便。"},
      {"step": "第4步·带分数化假分数", "detail": "遇到带分数先化成假分数再乘。例如：1½×2/3 → 3/2 × 2/3 → 先约分(3/2×2/3=1) → 1。"}
    ],
    "commonMistakes": [
    ...
    "aiDiagnosis": {
      "trigger": "做错时弹出",
      "action": "分数乘法卡住？排查三步：①分子乘分子、分母乘分母做对了吗？②乘完之后约分了吗（答案必须是最简分数）？③遇到带分数先化假分数了吗？",
      "interactionType": "choice-question"
    }
  }
,
...
,
  {
    ...
    "solveSteps": [
      {"step": "第1步·算总量", "detail": "把所有数据值加在一起。例如：语文92、数学88、英语96 → 92+88+96=276"},
      {"step": "第2步·数份数", "detail": "确定一共有几组数据参与平均。上例中3门课，份数=3"},
      {"step": "第3步·总量÷份数", "detail": "用第1步的和除以第2步的份数：276÷3=92（平均数）"},
      {"step": "第4步·读图验证", "detail": "条形图中找到每组的数据值，与计算平均值结果对比，确保一致"}
    ],
    "commonMistakes": [
    ...
    "aiDiagnosis": {
      "trigger": "卡住时提供",
      "action": "你在这里卡住，通常不是不会加除法，而是漏看'需要平均的是几份'。解题关键：先圈出题目里所有要参与平均数计算的数据值，再数一数有几个，最后用总量除以份数。",
      "interactionType": "dynamic-question"
    }
  }
,
